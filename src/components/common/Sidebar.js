...
export function getAllPermissions() {
    ...
    // Parse and return permissions, or return an empty array if not found
...
function Sidebar(props) {
    ...
    useEffect(() => {
        // Retrieve all permissions when the component mounts
                ...
                <ul className="menu-list flex-grow-1 mt-3">
                    {
                        menuData.map((d, i) => {
                            ...
                            if (d.permit === permissions[0] || d.permit === permissions[1]) {
                            ...
                            } else if(d.permit === 'management' || d.permit === 'dashboard'){
                                return <li key={"shsdg" + i} className=" collapsed">
                                <a className={`m-link ${d.children.filter((d) => "/" + d.routerLink[0] === activekey).length > 0 ? "active" : ""}`} href="#!" onClick={(e) => { e.preventDefault(); openChildren("menu-Pages" + i) }}>
                                    <i className={d.iconClass}></i>
                                    <span>{d.name}</span>
